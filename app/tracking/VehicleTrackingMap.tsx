...
interface TrackedDevice {
  ...
  /** GPS server id; may differ from `id` — match either when filtering by app `tracker_id`. */
...
function trackerIdsFromMyTractorsPayload(payload: unknown): Set<string> {
  const allowed = new Set<string>();
  const rows = Array.isArray(payload) ? payload : [];
  for (const row of rows) {
    if (!row || typeof row !== "object") continue;
    const id = (row as { tracker_id?: unknown }).tracker_id;
    if (id != null && id !== "") allowed.add(String(id));
  }
  return allowed;
}

/** When the user has no verified tractors (or payload empty), show all devices from the tracking account. */
function filterDevicesByMyTractorTrackerIds(
  devices: TrackedDevice[],
  myTractorsPayload: unknown
): TrackedDevice[] {
  const allowed = trackerIdsFromMyTractorsPayload(myTractorsPayload);
  if (allowed.size === 0) return devices;
  return devices.filter((d) => {
    const candidates = [d.id, d.traccar_device_id].filter(
      (x): x is number => x != null && !Number.isNaN(x)
    );
    return candidates.some((c) => allowed.has(String(c)));
  });
}
...
const [isLoaded, setIsLoaded] = useState(false);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (DATA_MODE === "dummy") {
          ...
          const allDevices = flattenDevicesFromGetDevicesBody(
          ...
          console.log("allDevices (flattened)", allDevices.length);
          const filteredAllDevices = filterDevicesByMyTractorTrackerIds(
            allDevices,
            response?.data
          );
          console.log("filteredAllDevices", filteredAllDevices.length);
    ...
    // Must not wait for map `isLoaded`: while `loading` is true we return early and never
    // mount MapContainer, so MapReadyHandler never runs and fetchData never starts (deadlock).
    fetchData();
        ...
        updatedDevices.forEach(device => {
          ...
          if (marker) {
            ...
            if (hasMoved) {
              ...
              const from = prevPosition || (currentLatLng ? { lat: currentLatLng.lat, lng: currentLatLng.lng } : newPosition);
  ...
  }, [map, userToken, generateInfoContent, animateMarker]);
  ...
  return (
    <div style={{ display: "flex", gap: isFullscreen ? "0" : "10px", height: isFullscreen ? "100vh" : "auto" }}>
      ...
      <div style={{ flex: 1, height: isFullscreen ? "100vh" : "auto" }}>
        ...
        >
          {/* Map paints in an isolated low stack; UI sits above Leaflet panes (tile ~200, overlay ~400, controls ~1000). */}
          <div
            style={{
              position: "relative",
              zIndex: 0,
          ...
          >
        <LeafletStrictModeGate style={{ width: "100%", height: "100%" }}>
        <MapContainer
        ...
        >
          ...
          {/* Float above Leaflet (panes use z-index up to ~1000) */}
          <div
            style={{
              position: "absolute",
              left: 0,
