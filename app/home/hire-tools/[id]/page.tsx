...
export default function HireToolPage({ params }: { params: { id: string } }) {
  ...
  return (
    <SidebarWithHeader isAuth={true}>
      <HireTractorForm id={id} groupId={groupId} />
    </SidebarWithHeader>
  );
}
...
function HireTractorForm({ id, groupId }: { id: string, groupId: string }) {
    ...
    const { userToken } = useAppSelector((state) => state.auth);
    const searchParams = useSearchParams();
    const farmSizeFromUrl = searchParams.get('farm_size');
    ...
    width="100%"
              ...
              <Flex
                ...
                width="100%"
              >
                <Field name="ward_district" validate={validateEmpty}>
                  {({ field, form }: { [x: string]: any }) => (
                    <FormControl
                      isInvalid={
