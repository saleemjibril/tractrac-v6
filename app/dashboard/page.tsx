...
export default function Dashboard() {
  ...
  const handleGetPayments = async (status?: string) => {
    setLoading(true);
    try {
      console.log("here");

      const response = await getMyHiredTractors(userToken as string);
      console.log("getAdminPayments", response);
      let data = response?.data?.items;
      setPayments(data);
      setPendingBookings(data?.filter((item: any) => item?.status === "pending" || item?.status === "payment_pending" || item?.status === "in_progress" || item?.status === "approved"));
    } catch (err) {
...
>
        ...
        <Box
          ...
          >
            ...
            <Link href="/dashboard/hired-tractors">
              ...
              >
                <Flex gap={"8px"} alignItems={"center"}>
                  ...
                  <Box
                    ...
                    bg={"#FAF6F6"}
                  >
                    ₦{formatAmount(payment?.total_amount?.toString())}
                  </Box>
                </Flex>
