...
import { getAgroToolInvoiceDetails, initialiseAgroToolPayment, verifyAgroToolPayment } from "@/app/apis/payment";
...
export default function Pay() {
  ...
  const handleGetInvoiceDetails = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const response = await getAgroToolInvoiceDetails(invoice, userToken as string);
      console.log("getAgroToolInvoiceDetails", response);
...
function MakePaymentForInvoice({ data }: { data: Record<string, string> }) {
  ...
  const handleCallBack = async (transactionId: string) => {
    ...
    const response = await verifyAgroToolPayment(transactionId, userToken as string);
  ...
  const handlePay = async () => {
    ...
    const response = await initialiseAgroToolPayment(
      data?.hire_addon_id,
      data?.invoice_number,
      data?.total_amount,
      userToken as string
    ...
    console.log("initialiseAgroToolPayment", response);
