...
export function InvestmentsEntry() {
  const {
    data: results,
    error,
    isLoading,
  } = useGetEntriesQuery("invest");
    ...
    return results.filter(
      (result) =>
        isNumeric
          ...
          : result.fname?.toLowerCase().includes(searchValue.toLowerCase()) // Search by name if it's not a number
    );
  }
          ...
          <Table variant="simple" bgColor="white">
            ...
            <Tbody>
              {filterData(results?.data, search).map((result: any) => (
                <Tr key={result?.id}>
                  <Td>{result?.fname || "N/A"}</Td>
                  <Td>{result?.lname ?? "N/A"}</Td>
                  <Td>{result?.phone ?? "N/A"}</Td>
                  <Td>{result?.email ?? "N/A"}</Td>
                  <Td>{result?.gender ?? "Nil"}</Td>
                  <Td>{parseFloat(result?.amount ?? 0).toLocaleString()}</Td>
